...
code_search <- tibble::tibble(
  tag = c("MELCOR", "ARCON", "RADTRAD", "NRCDose3", "MACCS", "Sierra ATD")
) |>
  dplyr::mutate(
...
MELCOR <- nrcadams::search_advanced(
    search_field = "single_content_search:MELCOR",
    results_tag = "MELCOR",
    days_back = 7,
    content_search = TRUE
) 
ARCON <- nrcadams::search_advanced(
    search_field = "single_content_search:ARCON",
    results_tag = "ARCON",
    days_back = 7,
    content_search = TRUE
) 
RADTRAD <- nrcadams::search_advanced(
    search_field = "single_content_search:RADTRAD",
    results_tag = "RADTRAD",
    days_back = 7,
    content_search = TRUE
) |> dplyr::mutate(tag = "RADTRAD")
NRCDose3 <- nrcadams::search_advanced(
    search_field = "single_content_search:NRCDose3",
    results_tag = "NRCDose3",
    days_back = 7,
    content_search = TRUE
) 
MACCS <- nrcadams::search_advanced(
    search_field = "single_content_search:MACCS",
    results_tag = "MACCS",
    days_back = 7,
    content_search = TRUE
) |> dplyr::mutate(tag = "MACCS")
ATD <- nrcadams::search_advanced(
    search_field = "single_content_search:'%22SIERRA+ATD%22'",
    results_tag = "ATD",
    days_back = 7,
    content_search = TRUE
) 
...
Codes <- MELCOR |>
  dplyr::bind_rows(ARCON) |>
  dplyr::bind_rows(RADTRAD) |>
  dplyr::bind_rows(NRCDose3) |>
  dplyr::bind_rows(MACCS) |>
  dplyr::bind_rows(ATD) 
